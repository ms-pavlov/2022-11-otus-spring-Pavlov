...
class GenresRepositoryImplIntegrationTest {
    ...
    private static final Long EXPECTED_GENRES_COUNT = 2L;
    ...
    void getByName() {
        var result = repository.findByName(EXISTING_GENRES_NAME);
    ...
    void getById() {
        var result = repository.findById(EXISTING_GENRES_ID).orElse(null);
    ...
    void update() {
        var genre = repository.findById(EXISTING_GENRES_ID)
                .map(value -> {
    ...
    void findByNameNotExist() {
        var result = repository.findByName(OTHER_GENRES_NAME);
