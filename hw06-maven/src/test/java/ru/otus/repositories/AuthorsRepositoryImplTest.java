...
class AuthorsRepositoryImplTest {
    ...
    private static final Author TEST = new Author(AUTHOR_ID, AUTHOR_NAME, List.of());
    ...
    @Test
    @DisplayName("при поиске по id книги должен выполнить sql запрос к БД и вернуть список авторов по книге")
    void findByBookId() {
        when(
                entityManager.createQuery(
                        eq("SELECT b.authors FROM Book b WHERE b.id = :id"),
                        eq(Author.class)))
                .thenReturn(query);
        when(query.setParameter("id", EXISTING_BOOKS_ID))
                .thenReturn(query);
        when(query.getResultList())
                .thenReturn(TEST_LIST);

        var result = authorsRepository.findByBookId(EXISTING_BOOKS_ID);
        ...
        verify(entityManager, times(1))
                .createQuery(
                        eq("SELECT b.authors FROM Book b WHERE b.id = :id"),
                        eq(Author.class));
        verify(query, times(1)).setParameter(eq("id"), eq(EXISTING_BOOKS_ID));
    ...
    void findByName() {
        ...
        var result = authorsRepository.findByName(AUTHOR_NAME);
    ...
    @Test
    @DisplayName("при удалении выполнить sql запрос к БД и обновить контекст")
    void delete() {
        when(
                entityManager.createQuery(
                        eq("delete from authors where id = :id")))
                .thenReturn(query);
        when(query.setParameter("id", AUTHOR_ID))
                .thenReturn(query);

        authorsRepository.delete(AUTHOR_ID);

        verify(entityManager, times(1)).createQuery(eq("delete from authors where id = :id"));
        verify(query, times(1)).setParameter(eq("id"), eq(AUTHOR_ID));
    ...
    void existTrue() {
        when(
                entityManager.createQuery(
                        eq("select count(id) from authors where id = :id"),
                        eq(Long.class)))
    ...
    void count() {
        when(
                entityManager.createQuery(
                        eq("select count(id) from authors"),
                        eq(Long.class)))
                .thenReturn(countQuery);
        when(countQuery.getSingleResult()).thenReturn(TEST_INT);
